...
def generate_oscilloscope(output_name,vidfor,input_audio,audfor,fps, res_width, res_height,interpolation,thickness,compression, callback_function):
    ...
    audioLinterp = np.zeros((audioLShaped.shape[0],audioLShaped.shape[1]*interpolation)).astype(np.int16)
    audioRinterp = np.zeros((audioRShaped.shape[0],audioRShaped.shape[1]*interpolation)).astype(np.int16)
    print(f"shape audioLinterp {audioLinterp.shape}")
    print(f"shape audioRinterp {audioRinterp.shape}")
    if interpolation > 1:
        callback_function(i,n_frames, text_state = True, text_message = "Upsampling...")
        for i in range(n_frames):
            audioLinterp[i,:] = signal.resample(audioLShaped[i,:], len(audioLShaped[i,:])*interpolation).astype(np.int16)
            audioRinterp[i,:] = signal.resample(audioRShaped[i,:], len(audioRShaped[i,:])*interpolation).astype(np.int16)
        fs = fs*interpolation
        print(f"fs {fs}")
        size_frame = size_frame*interpolation
    else:
        audioLinterp = audioLShaped.astype(np.int16)
        audioRinterp = audioRShaped.astype(np.int16)
...
def generate_polar(output_name,vidfor,input_audio,audfor,channel,fps, res_width, res_height,note, interpolation,thickness,compression, callback_function):
    output_name = output_name + vidfor
    input_audio = input_audio + audfor
    ...
    erre = audio
    theta = np.linspace(0, polar_speed*len(erre)/fs, len(erre))
    print(erre)
    print(theta)
    print(f"erre {erre.shape}")
    print(f"theta {theta.shape}")
     
    audioL = erre*np.sin(theta)*31130 ## 32768*0.95
    audioR = erre*np.cos(theta)*31130 ## 32768*0.95
...
def note_to_polarSpeed(note):
    #A4 ---> 2764.55 (?????)
    frequency = note_to_frequency(note)*2764.55/440
...
class OscilloscopeWindow:
    ...
    def __init__(self, master):
        self.master = master
        self.master.title("Oscilloscope Visualizer v0.04 by Aaron F. Bianchi")
...
class PolarWindow:
    ...
    def __init__(self, master):
        self.master = master
        self.master.title("Polar Visualizer v0.01 by Aaron F. Bianchi")
    ...
    def perform_action(self):
        try:
            ...
            if error_flag == True:
                ...
                # Do something with the input values (replace this with your final action)
                generate_polar(output_name,vidfor,input_audio,audfor,channel,fps, res_width, res_height,note,interpolation,thickness,compression,self.update_loading_label)
...
# Main window
root = tk.Tk()
root.title("LSaO Visualizer v0.60")
...
row_num = 0
# Initial text label
initial_text = "Usage:\n- Place your audio file in the same folder this executable is.\n- Will export the video to that folder. [WILL OVERWRITE]"
if os.name == 'nt':
    print("Running on Windows")
    initial_text = initial_text + "\n- FFmpeg needs to be manually installed and added to PATH.\n- The default Windows video player isn't going to play the output videos. Try VLC."
elif os.name == 'posix':
...
# Button for option 6
button_option6 = tk.Button(root, image=image6, text="Polar", compound=tk.BOTTOM, command=option6, width=130, height=130)
